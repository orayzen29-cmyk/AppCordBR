...
app.put('/api/servers/:serverId', auth, (req, res) => {
  ...
  db.get('SELECT owner_id FROM servers WHERE id = ?', [req.params.serverId], (err, server) => {
    ...
    db.run('UPDATE servers SET name = ? WHERE id = ?', [name, req.params.serverId], (err) => {
...
app.get('/api/servers', auth, (req, res) => {
  db.all(`SELECT DISTINCT s.* FROM servers s
          LEFT JOIN server_members sm ON s.id = sm.server_id
          WHERE s.owner_id = ? OR sm.user_id = ?`, [req.userId, req.userId], (err, servers) => {
...
app.listen(PORT, () => {
  console.log(`\n========================================`);
  console.log(`  AppCord - Servidor Iniciado`);
  console.log(`  Criado por mvks.7`);
  console.log(`========================================`);
